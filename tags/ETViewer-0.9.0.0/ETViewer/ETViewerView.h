...
struct CColumnInfo
{
	...
	BEGIN_PERSIST_MAP(CColumnInfo)
		...
		PERSIST(id,"Id")
		PERSIST(iOrder,"Order")
	END_PERSIST_MAP();

	CColumnInfo(){width=100;visible=false;}
	CColumnInfo(int _id,char* n,int fmt,int w,bool v,int o){id=_id;format=fmt;width=w;visible=v;name=n;iSubItem=-1;iOrder=o;}
};
...
protected:
	...
	int FindText(const char* pTextToFind,int baseIndex,bool up,bool stopAtEnd=false);

	void SetFocusOnOwnerWindow();

	char *GetTraceText(SETViewerTrace *pTrace,CColumnInfo *pColumn,char *pAuxBuffer,unsigned nAuxLen);
...
public:
	...
	void GetTransferBuffer(int *pSize,char **buffer,bool bAllTraces);
	...
	void SetTraceFont(string sTraceFont,DWORD dwFontSize);
	void GetTraceFont(string *psTraceFont,DWORD *pdwFontSize);
