...
DWORD CSourceFileViewer::OpenFile(const char *pFile,int line,bool bShowErrorIfFailed)
{
	...
	if(hFile==INVALID_HANDLE_VALUE)
	{
		...
		char sTemp[1024];
		sprintf(sTemp,"Cannot open source file %s due to error %d",m_SourceFile,result);
		if(bShowErrorIfFailed){MessageBox(sTemp,"",MB_ICONSTOP|MB_OK);}
	}
	else
	{
		...
		{ 
			unsigned x=0,y=0;
			while(g_pKeywords[x]!=NULL){pKeywordLenght[x]=(int)strlen(g_pKeywords[x]);x++;}
			...
			if(ReadFile(hFile,pReadedBuffer,m_FileBufferLength,&bytesRead,NULL))
			{
				...
				strupr(strcpy(m_pFileBufferUpper,m_pFileBuffer));
...
BOOL CSourceFileViewer::OnInitDialog() 
{
	...
	LOGFONT logFont = {0};
	logFont.lfHeight = 10;
	wsprintf(logFont.lfFaceName,"Courier");
...
void CSourceFileViewer::UpdateLine()
{
	...
	char sTemp[1024];
	sprintf(sTemp,"Ln %d,Col %d",line+1,col+1);
...
void CSourceFileViewer::ShowFindDialog()
{
	long selBegin=0,selEnd=0,line=0,col=0;
	m_EDFile.GetSel(selBegin,selEnd);
	char *pBuffer=new char [selEnd-selBegin+1];
	m_EDFile.GetSelText(pBuffer);
	m_LastTextToFind=pBuffer;
	delete [] pBuffer;
...
LRESULT CALLBACK CSourceFileViewer::FileEditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	if(uMsg==WM_RBUTTONDOWN)
	{
		...
		int character=pThis->m_EDFile.SendMessage(EM_CHARFROMPOS,0,(DWORD)&P);

		if(character<begin || character>end || begin==end || begin>end)
		{
			begin=end=character;
			int line=pThis->m_EDFile.LineFromChar(character);
			if(line>=0)
			{
				long charIndex=character-pThis->m_EDFile.LineIndex(line);
				pThis->m_EDFile.GetLine(line,A,sizeof(A)-1);
				if(charIndex>=(long)strlen(A)){charIndex=0;}

				bool includebegin=0,includeend=0;
				char beginCharToFind='"';
				char *pBegin=NULL,*pEnd=strchr(A+charIndex,'"');
				if(pEnd==NULL){pEnd=strchr(A+charIndex,'>');beginCharToFind='<';}
				if(pEnd!=NULL)
				{
					pEnd[0]=0;
					pBegin=strrchr(A,beginCharToFind);
					if(pBegin)
					{
						pBegin++;//skip " character
						(*pEnd)=0;
						strncpy(fileToOpen,pBegin,sizeof(fileToOpen)-1);
					}
				}
			}
		...
		{
			char *pBuffer=new char [end-begin+1];
			pThis->m_EDFile.GetSelText(pBuffer);
			strncpy(fileToOpen,pBuffer,sizeof(fileToOpen)-1);
			delete [] pBuffer;
		...
		if(fileToOpen[0]!=0)
		{
			...
			char A[2000];
			sprintf(A,"Open '%s'",fileToOpen);
			MENUITEMINFO itemInfo={0};
			itemInfo.cbSize=sizeof(itemInfo);
			itemInfo.cch=(unsigned)strlen(A);
			...
			if(command==ID_OPEN_AS_SOURCE_FILE)
			{
				...
				_splitpath(pThis->m_SourceFile,drive,path,NULL,NULL);
				temp1=drive;
				temp1+=path;
				if(strcmp(temp1.c_str(),"")!=0){dirs.push_back(temp1);}
				...
				if(!theApp.OpenCodeAddress(fileToOpen,0,false))
				{
					for(x=0;x<(int)dirs.size();x++)
					{
						...
						strncpy(fullPath,dirs[x].c_str(),sizeof(fullPath)-2); //-2 for the additional slash
						DWORD len=(DWORD)strlen(fullPath);
						if(len)
						{
							if(fullPath[len-1]!='\\')
							{
						...
						strncat(fullPath,fileToOpen[0]=='\\'?fileToOpen+1:fileToOpen,sizeof(fullPath)-1);
...
bool CSourceFileViewer::FindAndDeleteAll(const char *pText)
{
	return false;
}

bool CSourceFileViewer::FindAndMarkAll(const char *pText)
{
	return false;
}
...
{
	unsigned textToFindLenght=(unsigned)strlen(pTextToFind);
	for(int x=(offset-textToFindLenght);x>=0;x--)
	{
		if(memcmp(pBuffer+x,(char*)pTextToFind,textToFindLenght)==0)
		{
			return pBuffer+x;
		}
	}
...
bool CSourceFileViewer::FindNext(const char *pTextToFind)
{
	...
	strcpy(textToFind,m_LastTextToFind.c_str());
	if(!m_bMatchCaseInFind){strupr(textToFind);}
	unsigned textToFindLenght=(unsigned)strlen(textToFind);

	char *pTextFound=NULL;
	...
	{
		 pText=strstr(pBufferToSearchIn+end,(char*)textToFind);
		 if(pText==NULL){pText=strstr(pBufferToSearchIn,(char*)textToFind);}
		 begin=pText-pBufferToSearchIn;
	}
	...
	if(pText==NULL)
	{
		...
		sTemp+=" was not found";
		pParent->MessageBox(sTemp.c_str(),"",MB_OK);
	}
...
std::string CSourceFileViewer::GetFile()
{
	return m_SourceFile;
}