...
void CSettingsDialog::OnSelectFont()
{
	...
	strcpy(logFont.lfFaceName, m_sTraceFont.c_str());
...
BOOL CSettingsDialog::OnInitDialog()
{
	...
	m_LWSourcePaths.InsertColumn(1,"Path",LVCFMT_LEFT,400,0);
...
void CSettingsDialog::OnOk()
{
	...
	for(x=0;x<m_LWSourcePaths.GetItemCount();x++)
	{
		...
		item.cchTextMax=sizeof(sTempText)/sizeof(char);
...
{
	char sTempPath[MAX_PATH]={0};
	BROWSEINFO browseInfo={0};
	browseInfo.hwndOwner=m_hWnd;
	browseInfo.lpszTitle="Select the source path to add";
	...
	if(pResult)
	{
		...
		int pathLen=strlen(sTempPath);
		if(pathLen)
		{
			if(sTempPath[pathLen-1]=='\\'){sTempPath[pathLen-1]=0;}
...
void CSettingsDialog::SwapItems(int index1,int index2) 
{
	...
	char item1Text[1024]={0},item2Text[1024]={0};
