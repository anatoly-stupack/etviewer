...
int CETViewerView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		m_ColumnInfo.clear();
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Text,"Text",LVCFMT_LEFT,8000,true,0));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_PIDTID,"PID-TID",LVCFMT_LEFT,100,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Time,"Time",LVCFMT_LEFT,110,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_TimeStamp,"TimeStamp",LVCFMT_LEFT,100,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Index,"Index",LVCFMT_RIGHT,60,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Source,"Source",LVCFMT_LEFT,300,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Component,"Component",LVCFMT_LEFT,100,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Method,"Method",LVCFMT_LEFT,100,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Level,"Level",LVCFMT_LEFT,100,false,-1));
		m_ColumnInfo.push_back(CColumnInfo(eETViewerColumn_Flag,"Flag",LVCFMT_LEFT,100,false,-1));
	}
...
void CETViewerView::UpdateColumns()
{
	...
	for(unsigned x=0;x<nVisibleColumnCount;x++)
	{
		...
		column.cchTextMax=sizeof(sColumnText);
		...
		{
			CColumnInfo *pColumn=&m_ColumnInfo[y];
			if(strcmp(pColumn->name.c_str(),sColumnText)==0)
			{
				pColumn->iSubItem=x;
...
void CETViewerView::OnNMRclick(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	for(unsigned x=0;x<m_ColumnInfo.size();x++)
	{
		...
		pColumnsMenu->InsertMenu(0xFFFFFFFF,MF_BYPOSITION|flags,x+1,(char*)m_ColumnInfo[x].name.c_str());
	}
	pMenu->AppendMenu(MF_POPUP,(UINT_PTR)pColumnsMenu->m_hMenu,"Columns");
...
void CETViewerView::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if(pActivate->iItem!=-1)
	{
		RECT R={0,0,1,1};
		HDC dc=::GetDC(m_hWnd);
		::DrawText(dc,"A",1,&R,DT_SINGLELINE|DT_CALCRECT);
		::ReleaseDC(m_hWnd,dc);
		...
		unsigned lenght=(unsigned)strlen(text);
...
void CETViewerView::GetTraceColors(SETViewerTrace *pTrace,COLORREF *pTextColor,COLORREF *pBkColor, HPEN *phPen,HBRUSH *phBrush)
{
	...
	const char *text=pTrace->trace.sText.c_str();
	...
	for(x=0;x<theApp.m_SplittedHighLightFilters.size();x++)
	{
		...
		const char *pFilterText=pFilter->GetText().c_str();
...
void CETViewerView::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = CDRF_SKIPDEFAULT;
	...
	if(pDraw->nmcd.dwDrawStage==(CDDS_ITEMPREPAINT | CDDS_SUBITEM))
	{
		...
		char auxTextBuffer[1024];
		char *pText=GetTraceText(pTrace,m_mVisibleColumns[pDraw->iSubItem],auxTextBuffer,sizeof(auxTextBuffer));
...
char *CETViewerView::GetTraceText(SETViewerTrace *pTrace,CColumnInfo *pColumn,char *pAuxBuffer,unsigned nAuxLen)
{
	pAuxBuffer[0]=0;
	...
	case eETViewerColumn_Text:
		strcpy(pAuxBuffer,pTrace->trace.sText.c_str());
		break;
	case eETViewerColumn_PIDTID:
		sprintf(pAuxBuffer,"[%4d]-[0x%04x]",pTrace->trace.dwProcessId,pTrace->trace.dwThreadId);
		break;
	case eETViewerColumn_Time:
		sprintf(pAuxBuffer,"%02d:%02d:%02d.%03d",(DWORD)pTrace->trace.systemTime.wHour,(DWORD)pTrace->trace.systemTime.wMinute,(DWORD)pTrace->trace.systemTime.wSecond,(DWORD)pTrace->trace.systemTime.wMilliseconds);
		break;
	case eETViewerColumn_TimeStamp:
		sprintf(pAuxBuffer,"%f",((double)pTrace->trace.timeStamp.QuadPart)/10000000.0);
		break;	
	case eETViewerColumn_Source:
		sprintf(pAuxBuffer,"%s(%d)",pTrace->trace.sSource.c_str(),pTrace->trace.dwLine);
		break;
	case eETViewerColumn_Index:
		sprintf(pAuxBuffer,"%d ",pTrace->trace.dwSequenceIndex);
		break;
	case eETViewerColumn_Component:
		strcpy(pAuxBuffer,pTrace->trace.sComponent.c_str());
		break;
	case eETViewerColumn_Method:
		strcpy(pAuxBuffer,pTrace->trace.sFunction.c_str());
		break;
	case eETViewerColumn_Flag:
		strcpy(pAuxBuffer,pTrace->trace.sFlag.c_str());
		break;
	case eETViewerColumn_Level:
		strcpy(pAuxBuffer,pTrace->trace.sLevel.c_str());
		break;
	}
	return NULL;
}
...
LRESULT CALLBACK CETViewerView::ListEditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
		...
		if(wParam=='F' && (pushedControl))
		{
			...
			if(begin!=end)
			{
				...
				if(strlen(text)!=0)
				{
					...
					strncpy(textToFind,text+begin,len);
					pThis->m_LastTextToFind=textToFind;
				}
			}
			pThis->BeginFind(pThis,pThis->GetListCtrl().m_hWnd,(char*)pThis->m_LastTextToFind.c_str());
		}
	}
	if(uMsg==WM_RBUTTONDOWN)
	{
		...
		int begin=0,end=0;
		char text[1024],codeAddress[1024]={0};
		POINT p;
		::GetCursorPos(&p);
		pThis->GetListCtrl().GetEditControl()->GetSel(begin,end);
		pThis->GetListCtrl().GetEditControl()->GetWindowText(text,1024);
...
{
	if(m_LastTextToFind==""){return false;}
	return FindNext(m_LastTextToFind.c_str());
}
	
bool CETViewerView::FindNext(const char *pText)
{	
	if(strcmp(pText,"")==0){return false;}
	...
	if(index==-1)
	{
		...
		string str=pText;
		str+=" was not found";
		pParent->MessageBox(str.c_str(),"ETViewer",MB_OK);
		return false;
	}
	else
	{
...
int CETViewerView::FindText(const char* pTextToFind,int baseIndex,bool up,bool stopAtEnd)
{
	char textToFind[1024];
	strcpy(textToFind,pTextToFind);
	if(!m_bMatchCaseInFind){strupr(textToFind);}
	...
	for(x=0;x<count;x+=increment)
	{
		...
		if(!m_bMatchCaseInFind){strupr(text);}
		if(strstr(text,textToFind)!=NULL){return index;}
...
bool CETViewerView::FindAndMarkAll(const char *pTextToFind)
{
	bool res=false;
	int currentIndex=0;
...
bool CETViewerView::FindAndDeleteAll(const char *pTextToFind)
{
	WaitForSingleObject(m_hTracesMutex,INFINITE);
...
void CETViewerView::OnSave() 
{
	...
	CFileDialog dialog(FALSE,"txt",NULL,OFN_PATHMUSTEXIST|OFN_OVERWRITEPROMPT,"Text Files(*.txt)|*.txt|Log Files(*.log)|*.log|All Files(*.*)|*.*");
	if(dialog.DoModal()==IDOK)
	{
		...
		{
			char sTemp[1024];
			sprintf(sTemp,"Cannot create '%s' due to error %d",dialog.GetOFN().lpstrFile,GetLastError());
			MessageBox(sTemp,"ETViewer",MB_ICONSTOP|MB_OK);
			return;
		}
...
void CETViewerView::GetTransferBuffer(int *pSize,char **buffer,bool bAllTraces)
{
	(*buffer)=NULL;
	(*pSize)=0;
	if(bAllTraces)
	{
		char temp[2048],columnText[2048];
		...
		*pSize=(count*1024);
		...
		if((*buffer)==NULL)
		{
			MessageBox("Cannot allocate enough memory to perform the operation","ETViewer",MB_ICONSTOP|MB_OK);
			return;
		}
		(*buffer)[0]=0;
		for(index=0;index<count;index++)
		{
			...
			for(int x=0;x<columns;x++)
			{
				...
				// -3 : \r\n\0
				templen+=_snprintf(temp+templen,sizeof(temp)-3-templen,x==0?"%s":"\t%s",columnText);
				(*buffer)[size]=0;
			}
			temp[templen]='\r';
			temp[templen+1]='\n';
...
void CETViewerView::OnBeginEdit(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	::DrawText(dc,"A",1,&singleCharRect,DT_SINGLELINE|DT_CALCRECT);
...
//		sprintf(pDispInfo->item.pszText,"%d",pDispInfo->item.iSubItem);
		...
		char *pText=GetTraceText(pTrace,pColumnInfo,pDispInfo->item.pszText,pDispInfo->item.cchTextMax);
		if(pText){strcpy(pDispInfo->item.pszText,pText);}
	}
...
class CSortPredicateAscendingSource:public CSortPredicate
{
	public:
		...
		{
			int bFileComp=strcmp(t1->trace.sSource.c_str(),t2->trace.sSource.c_str());
...
{
	bool bOk=LoadFrom(pFile,"TracePanel");
	return bOk;
}
bool CETViewerView::Save(CConfigFile *pFile)
{
	...
	return SaveTo(pFile,"TracePanel");
}
...
void CETViewerView::SetTraceFont(string sTraceFont,DWORD dwFontSize)
{
...
void CETViewerView::GetTraceFont(string *psTraceFont,DWORD *pdwFontSize)
{
