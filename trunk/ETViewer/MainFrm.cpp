...
DWORD g_dwRegisteredMessage=RegisterWindowMessage("ETVIEWER-IPC");
// CMainFrame
	...
	CCreateContext* pContext)
{
	...
	theApp.m_Controller.StartRealTime("ETVIEWER_SESSION",GetTracePane());
...
void CMainFrame::LookupError(const char *pErrorString)
{
	char finalMessage[2000]={0},message[2000]={0},errorStr[2000]={0};
	char *pValidChars="0x1234567890abcdefABCDEFX";
	unsigned  len=(unsigned )strlen(pErrorString),added=0;
	for(unsigned x=0;x<len;x++)
	{
		if(strchr(pValidChars,pErrorString[x])!=0)
		{
		...
		{
			// a invalid char after valid chars marks the end.
			if(added){break;}
		}
	}
	if(strcmp(pErrorString,errorStr)!=0){m_MainDialogBar.m_EDErrorLookup.SetWindowText(errorStr);}


	DWORD errorCode=0;
	bool ok=(errorCode=atoi(errorStr))!=0;
	if(!ok){ok=(sscanf(errorStr,"%x",&errorCode)!=0);}
	if(!ok){ok=(sscanf(errorStr,"x%x",&errorCode)!=0);}
	if(!ok){ok=(sscanf(errorStr,"0x%x",&errorCode)!=0);}
	if(errorCode==0){return;}

	bool descriptionFound=false;

	descriptionFound=(FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM|FORMAT_MESSAGE_IGNORE_INSERTS,NULL,errorCode,MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),(LPTSTR) &message,sizeof(message), NULL)!=0);

	if(descriptionFound)
	{
		sprintf(finalMessage,"%s=%s",errorStr,message);
		MessageBox(finalMessage,"ETViewer",MB_OK|MB_ICONINFORMATION);
	}
	else
	{
		sprintf(finalMessage,"Cannot find the error %s",errorStr);
		MessageBox(finalMessage,"ETViewer",MB_OK|MB_ICONSTOP);
	}
}
...
{
	char temp[1024]={0};
	int count=m_MainDialogBar.m_EDErrorLookup.GetWindowText(temp,sizeof(temp));
	LookupError(temp);
...
bool CMainFrame::OpenFile(const char *pFile,bool *pbKnownFileType)
{
	char sExtension[MAX_PATH]={0};
	_splitpath(pFile,NULL,NULL,NULL,sExtension);
	strupr(sExtension);
	if(strcmp(sExtension,".PDB")==0)
	{
		if(pbKnownFileType){*pbKnownFileType=true;}
		return theApp.OpenPDB(pFile,true);
	}
	else if(strcmp(sExtension,".C")==0 || strcmp(sExtension,".H")==0 || 
		strcmp(sExtension,".CPP")==0 || strcmp(sExtension,".INL")==0)
	{
		if(pbKnownFileType){*pbKnownFileType=true;}
		return theApp.OpenCodeAddress(pFile,0,true);
	}
	else if(strcmp(sExtension,".ETL")==0)
	{
		if(pbKnownFileType){*pbKnownFileType=true;}
...
void CMainFrame::OnDropFiles(HDROP hDropInfo)
{
	...
	{
		MessageBox("Unknown files dropped","ETViewer",MB_ICONSTOP|MB_OK);
	}
...
{
	char sTempBuffer[1024*10]={0};
	CFileDialog dialog(TRUE,"pdb",NULL,OFN_ALLOWMULTISELECT|OFN_FILEMUSTEXIST,"All Supported Files|*.pdb;*.etl;*.c;*.cpp;*.h;*.inl|PDB Files(*.pdb)|*.pdb|Event Tracing Log Files(*.etl)|*.etl|Source Files(*.c *.cpp *.h *.inl)|*.c;*.cpp;*.h;*.inl|All Files(*.*)|*.*|");
	dialog.GetOFN().lpstrFile=sTempBuffer;
	dialog.GetOFN().nMaxFile=sizeof(sTempBuffer);

	if(dialog.DoModal()==IDOK)
	{
		string sPath;
		char *pString=dialog.GetOFN().lpstrFile;
...
{
	CFileDialog dialog(FALSE,"etl",NULL,OFN_PATHMUSTEXIST|OFN_OVERWRITEPROMPT,"Event Tracing Log Files(*.etl)|*.etl|All Files(*.*)|*.*");
	if(dialog.DoModal()==IDOK)
	{
...
{
	char sCommandLine[1024*10]={0};
	SetForegroundWindow();
	BringToTop(SW_SHOW);
	GlobalGetAtomName(wParam,sCommandLine,sizeof(sCommandLine));
	char *pArgs[]={sCommandLine};
