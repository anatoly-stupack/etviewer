...
void CProviderTree::UpdateProviderSubTree(HTREEITEM hProviderItem)
{
	...
	HTREEITEM hTraceFlagItems=treeCtrl.InsertItem("Flags",m_iFlagsIcon,m_iFlagsIcon,hProviderItem,TVI_LAST);
	...
	HTREEITEM hTraceLevelItems=treeCtrl.InsertItem("Levels",m_iLevelIcon,m_iLevelIcon,hProviderItem,TVI_LAST);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_FATAL",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_FATAL);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_ERROR",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_ERROR);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_WARNING",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_WARNING);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_INFORMATION",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_INFORMATION);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_VERBOSE",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_VERBOSE);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_RESERVED6",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_RESERVED6);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_RESERVED7",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_RESERVED7);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_RESERVED8",0,0,hTraceLevelItems,TVI_LAST);
	treeCtrl.SetItemData(hTempItem,TRACE_LEVEL_RESERVED8);

	hTempItem=treeCtrl.InsertItem("TRACE_LEVEL_RESERVED9",0,0,hTraceLevelItems,TVI_LAST);
...
{
	CTreeCtrl &treeCtrl=GetTreeCtrl();
	char A[1024]={0},B[1024]={0},file[MAX_PATH]={0},ext[MAX_PATH]={0};
	strcpy(A,pProvider->GetFileName().c_str());
	_splitpath(pProvider->GetFileName().c_str(),NULL,NULL,file,ext);
	strcat(B,pProvider->GetComponentName().c_str());
	strcat(B," - ");
	strcat(B,file);
	strcat(B,ext);
