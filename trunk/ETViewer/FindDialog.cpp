...
string	CFindDialogClient::m_LastTextToFind;
...
void CFindDialogClient::BeginFind(CWnd *pParent,HWND owner,const char *pTextToFind)
{
	if(!m_pFindDialog)
	{
		m_hFindOwner=owner;
		m_pFindDialog=new CFindDialog(this);
		m_pFindDialog->Create(TRUE,"",NULL,m_bFindDirectionUp?0:FR_DOWN,pParent);
...
CFindDialog::CFindDialog(CFindDialogClient *pFindClient)
{
	...
	m_fr.lpstrFindWhat=const_cast<char*>(m_pFindClient->m_LastTextToFind.c_str());
...
void CFindDialog::OnFind() 
{
	...
	m_COTextToFind.GetWindowText(newText,sizeof(newText));
	...
	if(bAddComboString)
	{
		if(strcmp(newText,"")!=0)
		{
			char existingText[2048]={0};
			char tempText[2048]={0};
			...
			for(x=0;x<pCombo->GetCount();x++)
			{
				...
				if(strcmp(strupr(existingText),tempText)==0)
				{
...
bool CFindDialog::UpdateOptions() 
{
	...
	{
		MessageBox("At least one search option must be selected\r\n\r\n\"Find in Process Id / Name\" \r\n\"Find in Trace Text\"","ETViewer",MB_ICONSTOP|MB_OK);
		res=false;
	}
	return res;
}
...
void CFindDialog::SetText(const char *pTextToFind)
{
