...
bool CTraceController::Format(STraceEvenTracingNormalizedData *pData)
{
	...
	int currentLen=0;
	char *pTempBuffer=new char [sizeof(sTraceText)];
	memset(pTempBuffer,0,sizeof(sTraceText));
	...
	for(DWORD x=0;x<pFormatEntry->m_vElements.size();x++)
	{
		...
		case eTraceFormatElementType_Byte:
			...
			currentLen+=sprintf(sTraceText+currentLen,pElement->pFormatString,cValue);
			break;

		case eTraceFormatElementType_Word:
			wValue=*(SHORT*)pCurrentParam;
			pCurrentParam+=sizeof(SHORT);
			currentLen+=sprintf(sTraceText+currentLen,pElement->pFormatString,wValue);
			break;
		...
		case eTraceFormatElementType_Float:
			...
			currentLen+=sprintf(sTraceText+currentLen,pElement->pFormatString,fValue);
			break;

		case eTraceFormatElementType_Double:
			dValue=*(double*)pCurrentParam;
			pCurrentParam+=sizeof(double);
			currentLen+=sprintf(sTraceText+currentLen,pElement->pFormatString,dValue);
			break;
		...
		case eTraceFormatElementType_UnicodeString:
			{
				sprintf(pTempBuffer,"%ws",pCurrentParam);
				DWORD dwBytes=sprintf(sTraceText+currentLen,pElement->pFormatString,pTempBuffer);
				currentLen+=dwBytes;
				pCurrentParam+=(wcslen((WCHAR*)pCurrentParam)+1)*2;
			}
			break;
		case eTraceFormatElementType_ConstantString:
			strcpy(sTraceText+currentLen,pElement->pFormatString);
			currentLen+=(int)strlen(sTraceText+currentLen);
			break;
		}
	}
...
void CTraceController::InitializeRealTimeSession(const char *pSessionName)
{
	ULONG dwBufferLen=sizeof(EVENT_TRACE_PROPERTIES)+(ULONG)strlen(pSessionName)+1;
	...
	if(m_OSVersionInfo.dwMajorVersion<6)
	{
		m_bQPCTimeStamp=false;
	...
	m_ConsumerProperties.LoggerName=const_cast<char *>(m_sSessionName.c_str());
...
ULONG CTraceController::OpenLog(const char *pLogFileName,ITraceEvents *piEvents)
{
	...
	m_ConsumerProperties.LogFileName=const_cast<char *>(m_sLogFileName.c_str());
...
ULONG CTraceController::StartRealTime(char *pSessionName,ITraceEvents *piEvents)
{
	if(m_eSessionType!=eTraceControllerSessionType_None){return ERROR_ALREADY_EXISTS;}
...
void CTraceController::InitializeCreateLog(const char *pSessionName,const char *pFileName)
{
	ULONG dwBufferLen=sizeof(EVENT_TRACE_PROPERTIES)+((ULONG)strlen(pSessionName)+1)+1024;
	m_pSessionProperties=(EVENT_TRACE_PROPERTIES *)new char [dwBufferLen];
	memset(m_pSessionProperties,0,sizeof(EVENT_TRACE_PROPERTIES));
	m_pSessionProperties->Wnode.BufferSize = dwBufferLen;
	CoCreateGuid(&m_pSessionProperties->Wnode.Guid);
	...
	m_pSessionProperties->LoggerNameOffset=sizeof(EVENT_TRACE_PROPERTIES);
	m_pSessionProperties->LogFileNameOffset =sizeof(EVENT_TRACE_PROPERTIES)+(ULONG)strlen(pSessionName)+1;
	m_pSessionProperties->MaximumFileSize = 100;

	strcpy(((char*)m_pSessionProperties)+m_pSessionProperties->LogFileNameOffset,pFileName);
...
ULONG CTraceController::CreateLog(const char *pFileName,ITraceEvents *piEvents)
{
	if(m_eSessionType!=eTraceControllerSessionType_None){return ERROR_ALREADY_EXISTS;}

	m_sLogFileName=pFileName;
	m_eSessionType=eTraceControllerSessionType_CreateLog;
	m_sSessionName="ETVIEWER LOG TO FILE SESSION";
	m_piEventCallback=piEvents;
...
string CTraceController::GetFileName()
{
