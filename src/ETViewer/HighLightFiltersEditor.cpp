...
void CHighLightFiltersEditor::OnDestroy()
{
    ...
    {
        CHighLightFilter* pFilter = (CHighLightFilter*)m_LWFilters.GetItemData(x);
        delete pFilter;
    }
...
LRESULT CALLBACK CHighLightFiltersEditor::ListViewProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_KEYDOWN)
    {
        ...
        if (wParam == VK_DELETE)
        {
            ...
            if (sel != -1)
            {
                CHighLightFilter* pFilter = (CHighLightFilter*)pThis->m_LWFilters.GetItemData(sel);
...
void CHighLightFiltersEditor::OnFilterClicked(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    {
        CHighLightFilter* pFilter = ((CHighLightFilter*)m_LWFilters.GetItemData(index));
